...
import {
  Users, TrendingUp, AlertCircle, UserX, ThumbsUp, ThumbsDown, Calendar
} from "lucide-react";
...
const StatCards = React.memo(({ analytics }: { analytics: AnalyticsData }) => {
  ...
  const Card = ({ title, value, icon, color, desc, textColor }: any) => (
    <div className="stats shadow bg-base-100 border border-base-200 w-full">
      <div className="stat p-4">
        <div className={`stat-figure ${textColor || `text-${color}`}`}>{icon}</div>
        <div className="stat-title text-xs uppercase font-bold opacity-60">{title}</div>
        <div className={`stat-value text-2xl ${textColor || `text-${color}`}`}>{value}</div>
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 animate-in fade-in duration-500">
      <Card title="Kehadiran" value={`${summary.attendanceRate}%`} desc="Persentase" color="primary" icon={<Users size={24} />} />
      <Card title="Hadir" value={summary.totalHadir} desc="Siswa masuk" color="success" icon={<div className="badge badge-success badge-sm">H</div>} />
      <Card
        title="Absen"
        value={summary.totalSakit + summary.totalIzin}
        desc="Izin/Sakit" color="warning" icon={<AlertCircle size={24} />}
      />
      <Card title="Alfa" value={summary.totalAlpha} desc="Tanpa Ket." color="error" icon={<div className="badge badge-error badge-sm">A</div>} />
      <Card title="Bolos" value={summary.totalBolos} desc="Bolos" textColor="text-purple-500" icon={<UserX size={24} className="text-purple-500" />} />
    </div>
  );
});

const ChartSection = React.memo(({ analytics, showRank }: { analytics: AnalyticsData, userType: any, showRank: boolean }) => {
  return (
          ...
          <div className="h-62.5 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={analytics.trend}>
                ...
                <Tooltip contentStyle={{ backgroundColor: COLORS.bg, borderRadius: '8px', border: 'none' }} itemStyle={{ color: '#fff' }} />
                <Area type="monotone" dataKey="hadir" stroke={COLORS.hadir} fillOpacity={1} fill="url(#colorHadir)" strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div className="h-62.5 w-full">
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none mt-4">
              <div className="text-center">
                <span className="text-2xl font-bold">{analytics.summary.attendanceRate}%</span>
      ...
      <div className="card bg-base-100 shadow-sm border border-base-200 col-span-1 lg:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-4 p-4">
        <div className="flex items-center justify-between bg-success/10 p-4 rounded-xl border border-success/20">
          <div>
            ...
            <p className="text-xl font-bold">{analytics.insights.mostProductiveDay.day}</p>
            <span className="badge badge-success badge-sm mt-1">{analytics.insights.mostProductiveDay.percentage}% Hadir</span>
          </div>
        ...
        <div className="flex items-center justify-between bg-error/10 p-4 rounded-xl border border-error/20">
          <div>
            ...
            <p className="text-xl font-bold">{analytics.insights.leastProductiveDay.day}</p>
            <span className="badge badge-error badge-sm mt-1">{analytics.insights.leastProductiveDay.percentage}% Hadir</span>
          </div>
            ...
            <div className="h-50 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={analytics.classPerformance} layout="vertical" margin={{ left: 10, right: 10 }}>
                  ...
                  <Tooltip cursor={{ fill: 'transparent' }} contentStyle={{ backgroundColor: COLORS.bg }} />
                  <Bar dataKey="percentage" fill={COLORS.hadir} radius={[0, 4, 4, 0]} barSize={15} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
...
export default function RekapRoute() {
  ...
  const handleDateChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (value !== "custom") {
      ...
      else if (value === "week") newStart = "date('now', 'weekday 1', '-7 days')";
  ...
  return (
    <div className="min-h-screen bg-base-200 flex flex-col font-sans pb-24">
      ...
      <main className="grow p-4 md:p-8 space-y-8 max-w-7xl mx-auto w-full">
        ...
        <div className="flex flex-col md:flex-row justify-between gap-4">
          <div>
            ...
            <p className="text-base-content/60 text-sm">
              Analitik performa {isPending ? <span className="loading loading-dots loading-xs"></span> : 'siap'}
            </p>
          </div>

          <div className="flex flex-wrap gap-2 w-full md:w-auto">
            <select onChange={handleDateChange} className="select select-bordered select-sm w-full md:w-auto">
            ...
            <select
              onChange={(e) => setKelasId(e.target.value === "all" ? undefined : parseInt(e.target.value))}
            ...
            <div className="collapse-content">
              <div className="flex flex-col md:flex-row justify-center gap-4 pt-2">
                <div className="text-center">
                  ...
                  <DayPicker mode="single" selected={startDatePicker} onSelect={setStartDatePicker} className="border rounded-lg p-2 bg-base-100 text-sm shadow-sm" />
                </div>
                <div className="text-center">
                  <span className="label-text font-bold">Sampai</span>
                  <DayPicker mode="single" selected={endDatePicker} onSelect={setEndDatePicker} className="border rounded-lg p-2 bg-base-100 text-sm shadow-sm" />
                </div>
              </div>
            </div>
          </div>
        ...
        {deferredAnalytics && (
          <div className={`space-y-6 transition-all duration-300 ${isPending ? 'opacity-50 grayscale-[0.5] scale-[0.99]' : 'opacity-100 scale-100'}`}>
            ...
            {/* 3. Table Data Mentah */}
            <div className="divider text-xs uppercase tracking-widest opacity-50">Data Detail</div>
            ...
            <div className="card bg-base-100 shadow-xl border border-base-200 overflow-hidden min-h-100">
              <div className="overflow-x-auto p-1">
                {/* CATATAN PENTING:
                           ...
                           jika row > 100. Jika tidak, tetap akan berat di DOM.
                        */}
                <Rekap
                  full={true}
                  type="onemonth"
