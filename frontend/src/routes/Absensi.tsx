...
export default function Absensi() {
  ...
  const today = new Date(); // Reference untuk validasi bulan
  ...
  const handleToggleLock = (date: string, kelasId: number, isLocked: boolean) => {
    const action = isLocked ? "Buka Kunci" : "Kunci Absensi";
    const fn = isLocked ? unlockAbsensi : lockAbsensi;
    
    Swal.fire({
      title: `${action}?`,
      text: isLocked ? "Data bisa diedit kembali." : "Data tidak akan bisa diedit.",
      icon: "question",
      ...
      <main className="flex-grow p-4 md:p-6 flex flex-col h-full overflow-hidden">
        <div className="max-w-5xl w-full mx-auto flex flex-col gap-4 h-full">
          ...
          <div className="flex-none flex flex-row justify-between items-center bg-base-100 p-3 md:p-4 rounded-2xl shadow-sm border border-base-300">
            <div>
              <div className="text-[10px] md:text-xs font-bold text-base-content/50 uppercase tracking-wider">Kelas</div>
              <div className="text-xl md:text-2xl font-bold text-primary truncate max-w-[150px]">{kelasName || "..."}</div>
            </div>

            {/* Month Navigator */}
            <div className="flex items-center bg-base-200 rounded-xl p-1 border border-base-300/50">
              <button onClick={() => changeMonth(-1)} className="btn btn-sm btn-ghost btn-square">
              ...
              <div className="px-3 text-center w-[120px] md:w-[140px]">
                <div className="font-bold text-sm md:text-base">{monthNamesFull[viewDate.getMonth()]}</div>
                <div className="text-[10px] text-base-content/60">{viewDate.getFullYear()}</div>
              </div>
          ...
          <div className="card bg-base-100 shadow-xl rounded-2xl border border-base-300 flex-grow flex flex-col overflow-hidden">
            ...
            <div className="flex-none grid grid-cols-12 gap-2 px-4 py-3 bg-base-100 border-b border-base-200 z-20 font-bold text-sm text-base-content/70 sticky top-0 shadow-sm">
              ...
              <div className="col-span-3 md:col-span-4 text-right pr-2">Aksi</div>
            </div>
            ...
            <div className="flex-grow overflow-y-auto scrollbar-thin pb-20">
               ...
               {filteredDates.map((dateString, i) => {
                  if (!db) return null;
                  ...
                  const siswas = getSiswa({ db, whereQuery: `kelas_id=${kelas}` });
                  ...
                  const totalTidakMasuk = absensies.filter((a) => a.status !== "hadir").length;
                  const isComplete = absensies.length >= siswas.length && siswas.length > 0;
                  
                  let locked = false;
                  if (kelas) locked = getIsLocked({ db, date: dateString.split(", ")[1], kelas_id: kelas });
                    ...
                    >
                      ...
                      <div className="col-span-4 md:col-span-3 pl-2">
                         <div className="flex flex-row items-center gap-3">
                            {/* Tanggal Angka */}
                            <div className={`text-xl font-bold w-8 text-center ${isSunday ? "text-error" : "text-base-content"}`}>
                                {date}
                            </div>
                            ...
                            <div className="flex flex-col justify-center">
                              <div className="flex items-center gap-2">
                                  <span className={`text-xs font-bold uppercase ${isToday ? "text-primary" : "opacity-70"}`}>
                                    {day}
                                  </span>
                                  
                                  {/* === CAP HARI INI === */}
                                  {isToday && (
                                    <span className="badge badge-xs md:badge-sm badge-primary font-bold shadow-sm animate-pulse text-[9px]">
                                        HARI INI
                                    </span>
                                  )}
                              </div>
                              <span className="text-[10px] uppercase tracking-wider opacity-50">{month}</span>
                            </div>
                         </div>
                      </div>
                      ...
                      <div className="col-span-3 md:col-span-3 flex justify-center">
                        {isComplete ? (
                          <span className="badge badge-success font-bold badge-md text-gray-800 border-none shadow-sm">Selesai</span>
                        ) : (
                           <span className="badge badge-ghost badge-md opacity-50 bg-base-200">Belum</span>
                        )}
                      </div>
                      ...
                      <div className="col-span-2 md:col-span-2 text-center font-semibold text-sm">
                         {totalTidakMasuk > 0 ? (
                            <span className={totalTidakMasuk >= 5 ? "text-error" : "text-warning"}>{totalTidakMasuk}</span>
                        ...
                        {!isSunday && (
                          <>
                            ...
                            {user?.type === "wali_kelas" && (
                              <button 
                                onClick={() => kelas && handleToggleLock(dateString.split(", ")[1], kelas, locked)}
