        ...
        try:
            ...
            for row in ws.iter_rows(min_row=2, values_only=True):
                ...
                kelas_obj, _ = Kelas.objects.get_or_create(
                    name=kelas_name, defaults={"active": True}
                )

                # 4. Simpan Siswa
                Siswa.objects.create(fullname=nama, kelas=kelas_obj, nis=nis, nisn=nisn)
                success_count += 1

            messages.success(
                request,
                f"Import Selesai! Berhasil: {success_count}, Skipped (Duplikat): {skipped_count}",
            )
            return redirect("admin:import_siswa")
    ...
    # Jika method POST, proses export
        ...
        try:
            ...
            absensi_qs = Absensi.objects.filter(
                siswa__kelas=kelas, date__year=year, date__month=month
            )
            ...
            for idx, siswa in enumerate(siswas, 1):
                ...
                ws.cell(row=row_num, column=summary_start_col + 1).value = counts[
                    "sakit"
                ]
                ws.cell(row=row_num, column=summary_start_col + 2).value = counts[
                    "izin"
                ]
                ws.cell(row=row_num, column=summary_start_col + 3).value = counts[
                    "alfa"
                ]
                ws.cell(row=row_num, column=summary_start_col + 4).value = counts[
                    "bolos"
                ]
